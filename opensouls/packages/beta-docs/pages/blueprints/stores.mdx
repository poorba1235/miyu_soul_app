...
<Callout type="info" emoji="ℹ️">
  For a full featured soul using the markdown pipeline and a blueprint store, check out [Raggy](https://github.com/opensouls/experimental-souls/tree/main/souls/raggy-knows-open-souls).
</Callout>
...
# Example
bunx soul-engine stores pull defaultBucket
bunx soul-engine stores push defaultBucket
bunx soul-engine stores pull organization/myBucket
```

## Pipelines

The `@opensouls/pipeline` project makes it simple to get started creating your own pipelines. Currently there's only a filesystem pipeline that supports text. Other formats will be supported later.

The easiest way to get started with a pipeline is to install the markdown pipeline into your own soul.

```bash
bunx soul-engine install pipelines/markdown
```
...
```js filename="package.json"
{
  ...
  "scripts": {
    // ...
    "pipeline": "bunx tsx ./scripts/pipeline.ts"
  }
  // ...
}
```
