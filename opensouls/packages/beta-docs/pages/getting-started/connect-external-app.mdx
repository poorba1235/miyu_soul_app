...
Find the terminal that's running your Telegram app (attention: this is not the one running `soul-engine dev`!). Stop the app by pressing `Ctrl+C` in the terminal, and then run it again with `bunx tsx telegram/index.ts`. Now, when you send a message to the Graham, it should be his soul that replies:
...
import { MentalProcess, useActions, useSoulMemory } from "@opensouls/engine";
import externalDialog from "./cognitiveSteps/externalDialog.ts";
import reengage from "./mentalProcesses/reengage.ts";
