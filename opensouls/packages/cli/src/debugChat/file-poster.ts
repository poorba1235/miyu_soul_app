...
import { getConnectedWebsocket, websocketUrl } from "../sockets/soul-engine-socket.ts";
...
export class FilePoster extends EventEmitter<FilePosterEvents> {
  ...
  private async setupProvider() {
    ...
    const wsUrl = websocketUrl(organizationSlug, Boolean(local), false)
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Timeout connecting to soul-engine server at ${wsUrl}. Is it running?`))
      }, 15_000)
