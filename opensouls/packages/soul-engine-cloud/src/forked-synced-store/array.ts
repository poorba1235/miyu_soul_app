...
import { areSame, getYjsValue, INTERNAL_SYMBOL } from "./index.ts";
import { Box } from "./boxed.ts";
import { crdtValue, ObjectSchemaType, parseYjsReturnValue } from "./internal.ts";
import { CRDTObject } from "./object.ts";
...
function arrayImplementation<T>(arr: Y.Array<T>) {
  ...
  // (topper): this is my forked version of the original implementation
  const ret = [];
